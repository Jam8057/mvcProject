					...
					<tbody>
						<c:choose>
							...
							<c:when test="${empty boardList}">
								<tr>
									<td colspan="5" aligh="center">게시물이 없습니다.</td>
								</tr>
							</c:when>
							<c:when test="${!empty boardList }">
				...
				<ul class="pagination">
					...
					<c:forEach begin="${pagination.startPage}"
						...
						<li
							...
							onClick="fn_pagination('${idx}', '${pagination.range}', '${pagination.rangeSize}', '${search.searchType}', '${search.keyword}')">${idx}
						</a></li>
					</c:forEach>
					<c:if test="${pagination.next}">
						<li class="page-item"><a class="page-link" href="#"
						onClick="fn_next('${pagination.range}', '${pagination.range}', '${pagination.rangeSize}','${search.searchType }', '${search.keyword }')">Next</a></li>
					</c:if>
				</ul>
			</div>
			<!-- pagination{e} -->
